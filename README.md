...
**Обмен между компьютерами.** Второй ПК с LanSync появляется в списке сам — компьютеры
находят друг друга в сети. Один раз показываете код на принимающем, вводите на отправляющем
— и дальше выбранные файлы уходят кнопкой «→ имя ПК» прямо в его каталог обмена, с прогрессом
и отменой. Текстовые записи — так же.
...
| `LANSYNC_MDNS` | вкл. | `0` — не анонсировать сервис через mDNS. Соседние компьютеры при этом и не ищутся: рассылка одна и та же. Привязать соседа по адресу вручную всё равно можно |
...
- Запросы **с самого компьютера** проходят без токена — иначе пришлось бы копировать секрет
  в собственный браузер. Своим считается только `localhost`, поэтому страница, открытая
  на ПК по сетевому адресу машины, сама перенаправляется на `localhost`: по адресу источника
  такой запрос неотличим от телефона и иначе упирался бы в запрос токена.
...
Соседний ПК с LanSync виден в блоке «Компьютеры в сети». Чтобы разрешить ему отправку:
на принимающем нажмите «Показать код», на отправляющем — «Привязать» и введите код. Код
шестизначный, живёт пять минут и работает один раз.

Привязка односторонняя: она разрешает отправку в одну сторону. Нужно в обе — покажите код
на каждой машине по очереди. Отправляющая сторона запоминает соседа насовсем; принимающая
видит его обычной записью в списке устройств и отзывает той же кнопкой «Отвязать».

Если сосед не появился сам, добавьте его по адресу («Добавить по адресу»): в гостевых и
офисных сетях рассылку mDNS часто запрещают, а прямое соединение при этом работает.

Отдельно про сертификат: отпечаток соседа запоминается при привязке и проверяется при каждой
отправке. Сертификат перевыпускается сам при смене адресов компьютера — тогда LanSync
покажет «сертификат изменился» и приостановит отправку, пока вы не подтвердите, что это та
же машина.
...
4. **Не тот адрес.** Если у компьютера несколько сетей, под QR-кодом появляются кнопки со
   всеми его адресами. Выберите тот, что относится к сети телефона, — QR-код перестроится.
5. **Старая закладка на `http://`.** После включения HTTPS прежние ссылки не работают:
   отсканируйте QR-код заново.

## Если сети нет вообще

Роутер не обязателен: достаточно, чтобы устройства оказались в одной сети, а раздать её
может любое из них.

**Телефон раздаёт, компьютер подключается.** Самый надёжный способ. Включите на телефоне
точку доступа («Режим модема»), подключите к ней компьютер — дальше всё как обычно.
Мобильный интернет для этого не нужен: раздаётся сеть, а не трафик. На iPhone режим модема
без действующего тарифа может быть недоступен.

**Компьютер раздаёт, телефон подключается.**

```bash
nmcli device wifi hotspot ssid LanSync password ваш-пароль   # Linux, адрес хоста 10.42.0.1
```

На Windows — «Параметры» → «Сеть и Интернет» → «Мобильный хот-спот». Две оговорки: карта
должна поддерживать режим точки доступа, и большинство карт не умеют одновременно быть
точкой доступа и клиентом — текущее подключение к Wi-Fi отвалится. Сеть точки доступа —
новая, поэтому Windows заново спросит про её категорию, и правило брандмауэра должно
совпасть с ответом.

**Два компьютера без роутера.** Проще всего подключить оба к точке доступа телефона —
телефон работает роутером и в обмене не участвует. Второй путь — соединить их кабелем
(современным сетевым картам кроссовер не нужен). DHCP-сервера в такой сети нет, и системы
назначают себе адреса вида `169.254.*` сами. LanSync их принимает: когда обычных адресов
нет, он показывает в QR-коде и ищет соседей именно по ним. На Linux может понадобиться
разрешить это в профиле соединения:

```bash
nmcli connection modify «имя» ipv4.method link-local
```

Чего сделать нельзя: обойти **изоляцию клиентов** в чужой сети. Если гостевой Wi-Fi в кафе
или отеле запрещает устройствам видеть друг друга, поможет только своя точка доступа.
Wi-Fi Direct и Bluetooth браузеру недоступны, поэтому в LanSync их нет.
...
|---|---|---|
| `.github/workflows/ci.yml` | каждый push и pull request | типы, 30 тестов, синтаксис и связность клиента — на Windows, Linux и macOS; затем пробная сборка исполняемого файла на каждой из них |
